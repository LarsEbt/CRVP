    ...
    public partial class Form1 : Form
    {
        ...
        private void bMultiLoad_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (var solPath in openFileDialog1.FileNames)
                {
                    var file = Path.GetFileNameWithoutExtension(solPath);
                    var parts = file.Split('_');
                    if (parts.Length < 3)
                    ...
                    try
                    {
                        ...
                        var demands = instance.Demands;
                        double totalDemand = 0;
                        int vehiclesUsed = 0;
                        for (int k = 0; k < K; k++)
                        {
                            if (solution.YSol[k] < 0.5) continue;
                            vehiclesUsed++;
                            double routeDemand = 0;
                            for (int i = 1; i < n; i++)
                            {
                                if (solution.XSol[k, i] > 0.5)
                                    routeDemand += demands[i];
                            }
                            totalDemand += routeDemand;
                        }
                        double avgUtil = vehiclesUsed > 0 ? (totalDemand / (vehiclesUsed * instance.VehicleCapacity)) * 100 : 0;
                        dataGridView1.Rows.Add(
                            instanceName,
                            solution.Solver,
                            solution.TravelCosts.ToString("F2"),
                            solution.SolutionTime.ToString("F2"),
                            vehiclesUsed.ToString(),
                            totalDemand.ToString("F1"),
                            avgUtil.ToString("F1") + "%"
                        );
                    }
                    catch { }
        ...
        private void PopulateRouteList()
        {
            ...
            textBox6.Text = $"{currentSolution.SolutionTime:F2}";
        }
